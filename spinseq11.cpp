...
ll gx, gy, gden=2 ;
void nextgen(double &x, double &y) {
   while (1) {
      gx++ ;
      if (gx >= gden) {
         gy++ ;
   ...
   for (; ; trial++) {
      ...
      for (int d=1; ; d++) {
       ...
       if (front.w.size() == 0)
          break ;
       for (auto &e: front.w) {
         if (abs(getdouble(e.p.first)) < 0.00001 && abs(getdouble(e.p.second)) < 0.00001)
       ...
       if (ord > maxord) {
          finalchar = '>' ;
          cout << "Trial " << trial << " " << ox << " " << oy << " " << ord << endl << flush ;
          goto bugout ;
       }
       if (ord > target) {
          cout << "> " << target << " " << ox << " " << oy << " in " << duration() << endl << flush ;
          target += target / 10 ;
       }
      }
bugout:
      if (userandom && verbose > 1)
         cout << "Trial " << trial << " " << ord << " deep " << deepest << endl << flush ;
      if (ord > deepest) {
         deepest = ord ;
         finalval = ord ;
         if (userandom)
